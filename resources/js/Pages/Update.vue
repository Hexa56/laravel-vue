<template>
    ...
    <div class="container flex justify-center mt-24 fixed h-full w-full ">
        <div class='flex flex-col w-96'>
            ...
            <input class="p-2 border m-2 rounded" type="text" v-model="data.description" placeholder="description">
            <input class="p-2 border m-2 rounded" type="number" v-model="data.amount" placeholder="amount"> 
            <select class="p-2 border m-2 rounded" v-model="data.category" id="" title="Select Category">
                <option v-for="cat in cats" :value="cat">{{cat}}</option>
            </select>
            <select class="p-2 border m-2 rounded" v-model="data.payment_method" id="" title="Payment_method">
                <option v-for="paymth in paymths" :value="paymth">{{paymth}}</option>
            </select>
            <button @click="submit(data)" class="bg-black text-white py-2 m-2 rounded">Add</button>
